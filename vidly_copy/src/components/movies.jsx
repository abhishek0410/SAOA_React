...
class Movies extends Component {
  ...
  handleDelete = movie => {
    const movie_update = this.state.movies.filter(
      temp_movie => temp_movie._id !== movie._id
    );
  ...
  handleListGroup = genre_name => {
    ...
    var movies_filtered = this.state.movies.filter(
      movies => movies.genre.name === genre_name.name
    );
